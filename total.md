...
- Чтобы развернуть проект, нужно скачать репозиторий [libs](http://hg.turbodevelopers.com/turbo/libs), перейти в крайнюю ревизию ветки `altVersion-ks` (для LESS) или `altVersion-Stylus` (для Stylus) и скопировать содержимое в папку нового проекта.
