...
Если вы хотите верстать на Jade, не забудьте поставить соответствующие npm пакеты из `templates/package.json` (см. [Разворачивание проекта](deployment.md)) и настроить `gulp-config.js` (см. [Сборка проекта](build.md)). Также можете удалить папку `templates/components/` с PHP компонентами, т.к. при верстке на jade они не понадобятся.
